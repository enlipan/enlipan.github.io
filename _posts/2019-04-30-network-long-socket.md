...
![long-link](http://img.javaclee.com/20190420003231.png)

双端维护虚拟的TCP 通道.  

TCP 自带维护链接的机制, keep-alive ,自动清除死链接, 尝试探测是否有数据通信,没有数据通信的链接将被清理.  

客户端心跳包发送, 服务端检测客户端在线状态, 用于缓存数据清理. 

方案一: 

探知 tcp的链接中断状态判断. socket 流数据获取最终状态.  
