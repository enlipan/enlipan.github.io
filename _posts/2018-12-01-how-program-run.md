...
RLE 算法数据压缩: 数据的重复次数: A2B3D4 表示. 压缩率由重复率决定,对于文本等重复率较小的内容不适用. 

摩尔斯编码  

哈夫曼编码: 内容 =压缩处理 =>  编码信息 + 内容  
...
硬件中的核心由 CPU 决定, CPU 指令集. CPU 只能解释执行自身固有的机器指令.  

操作系统完成了对底层硬件系统的封装,将其标准化. 

JVM : Java 虚拟机,提供不依赖于特定硬件与操作系统的统一程序运行环境.
JVM 将 class 信息转换为NativeCode 执行.
...
汇编语法: 单行指令= 操作码 + 操作数. 操作码代表指令动作,而操作数代表指令对象,也就是代表是向谁发出的指令.多个操作数之间用,隔开.

汇编语言是针对 cpu 的指令,能够使用何种操作码是由 cpu 规格定义的.


寄存器的功能区分: 主要有存储/ 运算功能. 累加器/循环计数器..

MOV/PUSH/POP 

函数调用机制:  call 指令是核心.  

全局变量与函数内部局部变量的差异: 在程序运行过程中,全局变量的内存空间一直得到保存.而局部变量被保存在寄存器与栈中,当函数执行完毕,所有相关信息被清理销毁,恢复到函数执行前的初始状态.由于寄存器的访问速度更快,因而函数执行过程中所需要的内存空间会尽可能的被分配在寄存器中.

局部变量在寄存器空间足够可用时优先使用寄存器,而当寄存器空间不足时则申请开辟栈空间.而栈空间一旦函数使用后,在函数退出时就需要清空,恢复初始状态,否则将会将栈空间消耗殆尽.

比较跳转: cmp 指令, 比较结果存储在标志寄存器中.

只有了解计算机执行指令的细节才能正确的看待细节问题,如多线程中的原子性.


### 应用与硬件操作

IO 操作. IO 控制器交换计算机主机与外部硬件设备之间的交互. 不同的 IO 控制器有不同的主机端口号.

IO 指令在指定的端口号与 CPU 之间流转交互传递.

IRQ: 中断请求.多任务交互时的处理,用于暂停当前正在运行的程序,跳转到其他程序继续执行的机制. 
...
中断通知是从轮询事件状态的一种事件改进机制,释放 CPU 资源. 


DMA : 不通过 CPU外围硬件与 主内存直接通信的方式. 

VRAM: 显示器信息专用内存区域. GPU 作为特殊的计算资源,加速 VRAM 中信息在显示器上的描绘显示.  